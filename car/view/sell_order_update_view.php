...
$query = 'SELECT item.name AS __iname, unit.name AS __uname, sell_order_item.unit_amount AS __ua, sell_order_item.final_per_unit_price AS __up
 FROM sell_order_item JOIN sell_order ON sell_order_item.order_id=sell_order.id
 JOIN item ON sell_order_item.item_id = item.id
 JOIN unit ON sell_order_item.unit_id = unit.id
 where sell_order_item.order_id='.$order_id;
...
$result = $du->getQueryData('SELECT paid,note from sell_order where id='.$order_id);
...
$u->beginForm("../update/sell_order_update.php");
