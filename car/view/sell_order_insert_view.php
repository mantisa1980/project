...
$t->clear();
$keys=  ["item_name","unit_name","sell_price"];
$names= ["品項","單位","購買價格"];
$t->setColumnFields($keys,$names);

$query = 'select item.name AS item_name, unit.name AS unit_name, price.sell_price, customer.name as __cname from price JOIN customer ON price.group_id = customer.group_id JOIN item on price.item_id=item.id JOIN unit ON price.unit_id=unit.id where customer.id ='.$customer_id.' ORDER BY item_name' ;
...
echo '
        <form id="form"  method="post" action="../insert/sell_order_insert.php">
            <h3>輸入訂單資料</h3>
