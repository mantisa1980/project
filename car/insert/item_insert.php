...
$link = connectCarDB();
$stmt = mysqli_prepare($link, "INSERT INTO item(`name`) VALUES (?)");
...
$query='SELECT groups.id AS group_id, unit.id AS unit_id FROM groups, unit GROUP BY group_id, unit_id';
...
foreach ($result as $i)
{
	...
	mysqli_stmt_bind_param($stmt, 'ddd', $group_id,$new_insert_id,$unit_id);
	$r = mysqli_stmt_execute($stmt);
	if(!$r)
	{
	    echo "mysql error:".mysqli_error($link)."<br>";
